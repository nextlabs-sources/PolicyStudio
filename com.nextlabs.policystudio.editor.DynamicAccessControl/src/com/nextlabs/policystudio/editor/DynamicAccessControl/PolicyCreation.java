...
public class PolicyCreation extends CreationExtension {
	...
	public void postCreation(IDPolicy policy) {
		...
		CompositePredicate userPred = new CompositePredicate(BooleanOp.AND, PredicateHelpers.buildConstantArrayList(true));
		CompositePredicate hostPred = new CompositePredicate(BooleanOp.AND, PredicateHelpers.buildConstantArrayList(true));
		CompositePredicate appPred = new CompositePredicate(BooleanOp.AND, PredicateHelpers.buildConstantArrayList(true));
		...
		CompositePredicate fromPred = new CompositePredicate(BooleanOp.AND, PredicateHelpers.buildConstantArrayList(true));
		target.setFromResourcePred(fromPred);

		CompositePredicate toPred = new CompositePredicate(BooleanOp.AND, PredicateHelpers.buildConstantArrayList(true));
		...
		CompositePredicate actPred = new CompositePredicate(BooleanOp.AND, PredicateHelpers.buildConstantArrayList(true));

		policy.setAttribute("DYNAMIC_ACCESS_CONTROL_POLICY", true);
