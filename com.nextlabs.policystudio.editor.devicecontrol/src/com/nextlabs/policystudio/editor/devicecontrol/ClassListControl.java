			...
			public void dragEnter(DropTargetEvent event) {
				...
				for (int i = 0; i < event.dataTypes.length; i++) {
					if (transfer.isSupportedType(event.dataTypes[i])) {
						...
						try {
							String in = null;
							if (spec[0] instanceof SpecReference) {
								SpecReference ref = (SpecReference) spec[0];
								if (ref.isReferenceByName()) {
									in = ref.getReferencedName();
								}
							} else if (spec[0] instanceof IRelation) {
								IExpression exp = ((IRelation) spec[0])
										.getLHS();
								in = exp.toString();
							}
							String con = getComponentType();
							if (in == null
									|| !PolicyServerHelper
											.getTypeFromComponentName(in)
											.equals(con)) {
								event.detail = DND.DROP_NONE;
								return;
							}
