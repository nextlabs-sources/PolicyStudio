...
public class CompositionControl extends Composite {
	 ...
	 * @param style
	 */
	public CompositionControl(Composite parent, int style, String name,
	...
	private void addExpressionControls(CompositePredicate spec) {
		...
		ClassListControl c = new ClassListControl(this, getEntityType(),
		...
		if (isEditable()) {
			...
			if (canBrowse()) {
				...
				if (lookupLabel != null
						&& (lookupLabel
								.equals(EditorMessages.SERVERCOMPONENTEDITOR_LOOKUP) || lookupLabel
								.equals(EditorMessages.PORTALCOMPONENTEDITOR_LOOKUP))) {
					...
					browseButton.addSelectionListener(new SelectionAdapter() {
						...
						public void widgetSelected(SelectionEvent e) {
							...
							if (dialog.open() != null) {
								...
								((ClassListControl) classListArray.get(index))
							...
							if (leafObjectBrowser.open() == Dialog.OK) {
								...
								for (String item : itemsToAdd) {
									IPredicate pred = new Relation(
											...
											ResourceAttribute.PORTAL_URL
													.build(item));
									((ClassListControl) classListArray
											.get(index))
											.addElementWithUndo(pred);
									((ClassListControl) classListArray
											.get(index)).relayout();
						...
						public void widgetSelected(SelectionEvent e) {
							...
							if (leafObjectBrowser.open() == Dialog.OK) {
								itemsToAdd = leafObjectBrowser
										.getItemsToReturn();
								((ClassListControl) CompositionControl.this.classListArray
										.get(index))
	...
	public void relayout() {
		...
		tabListArray.clear();
		for (ClassListControl control : classListArray) {
			control.relayout();
		}
		int spacing = 5;
