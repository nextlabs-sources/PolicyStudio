 ...
 * reserved worldwide.
 */
package com.nextlabs.policystudio.pdf.communication;
 ...
 *          .pdf.communication/src/com/nextlabs/policystudio/pdf/communication/
 *          CommunicationPolicyPdfGenerator.java#4 $
 */

public class CommunicationPolicyPdfGenerator extends PolicyPdfGenerator
		implements IPdfGenerator {
	public CommunicationPolicyPdfGenerator(IDPolicy policy,
	...
	public void generate() throws DocumentException {
		...
		Phrase phrase = new Phrase(getPolicyName() + " [Communication Policy]",
				font);
		...
		phrase = new Phrase("Communication between Sender(s): ", font);
		paragraph = new Paragraph();
		paragraph.setSpacingBefore(8);
		paragraph.add(phrase);
		...
		if (!policy.hasAttribute("access")) {
			...
			if (!applicationList.isEmpty()) {
				...
				if (isEmpty) {
					font = new Font(Font.TIMES_ROMAN, 12, Font.ITALIC);
					phrase = new Phrase("Using Applications", font);
					paragraph.add(phrase);
					document.add(paragraph);
					isEmpty = false;
				} else {
					font = new Font(Font.TIMES_ROMAN, 12, Font.ITALIC);
					phrase = new Phrase("And Using Applications", font);
					paragraph.add(phrase);
					document.add(paragraph);
				}

				getUserSubjectLabel(CommonPolicyConstants.operators1,
						applicationList);
			}
		}
		...
		// resource
		font = new Font(Font.TIMES_ROMAN, 12, Font.NORMAL);
		phrase = new Phrase("And Recipient(s): ", font);
		paragraph = new Paragraph();
		paragraph.setSpacingBefore(8);
		paragraph.add(phrase);
		document.add(paragraph);
		...
		// with attachment
		font = new Font(Font.TIMES_ROMAN, 12, Font.NORMAL);
		phrase = new Phrase("With Attachment(s): ", font);
		paragraph = new Paragraph();
		paragraph.setSpacingBefore(8);
		paragraph.add(phrase);
