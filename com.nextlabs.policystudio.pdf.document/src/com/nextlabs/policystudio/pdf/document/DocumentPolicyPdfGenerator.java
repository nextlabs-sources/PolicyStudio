 ...
 * reserved worldwide.
 */
package com.nextlabs.policystudio.pdf.document;
 ...
 *          .pdf.document/src/com/nextlabs/policystudio/pdf/document/
 *          DocumentPolicyPdfGenerator.java#4 $
 */

public class DocumentPolicyPdfGenerator extends PolicyPdfGenerator implements
		IPdfGenerator {
	public DocumentPolicyPdfGenerator(IDPolicy policy,
	...
	public void generate() throws DocumentException {
		...
		Phrase phrase = new Phrase(getPolicyName() + " [Document Policy]", font);
		...
		if (!policy.hasAttribute("access")) {
			...
			if (!applicationList.isEmpty()) {
				...
				if (isEmpty) {
					font = new Font(Font.TIMES_ROMAN, 12, Font.ITALIC);
					phrase = new Phrase("Applications", font);
					paragraph.add(phrase);
					document.add(paragraph);
					isEmpty = false;
				} else {
					font = new Font(Font.TIMES_ROMAN, 12, Font.ITALIC);
					phrase = new Phrase("And Applications", font);
					paragraph.add(phrase);
					document.add(paragraph);
				}
