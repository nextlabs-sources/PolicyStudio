 ...
 * @author fuad
 * @version $Id:
 *          //depot/main/Destiny/main/src/client/policymanager/src/java/main/com/bluejungle/destiny/policymanager/ui/controls/CReflectedCombo.java#4 $:
 */
 ...
 * </dl>
 */
public final class CReflectedCombo extends Composite {
	 ...
	 * @see Widget#getStyle()
	 */
	public CReflectedCombo(Composite parent, int style) {
		...
		if ((style & SWT.FLAT) != 0)
			textStyle |= SWT.FLAT;
		text = new CLabel(this, textStyle);
		// ADDED BY BLUEJUNGLE
		// text.setAlignment(SWT.RIGHT);
		text.setImage(new Image(getDisplay(),
				"C:\\policymanagerimages\\downarrow.bmp"));
		// this.text.setImage(PlatformUI.getWorkbench().getSharedImages().getImage(ISharedImages.IMG_TOOL_BACK));
		// ADDED BY BLUEJUNGLE
		...
		listener = new Listener() {
			...
			public void handleEvent(Event event) {
				...
				if (CReflectedCombo.this == event.widget) {
					comboEvent(event);
					return;
				}
		...
		filter = new Listener() {
			...
			public void handleEvent(Event event) {
				...
				if (shell == CReflectedCombo.this.getShell()) {
					handleFocus(SWT.FocusOut);
				}
			}
		};
	 ...
	 * @exception SWTError
	 *                (ERROR_ITEM_NOT_ADDED) when the item cannot be added
	 */
	...
	public Point computeSize(int wHint, int hHint, boolean changed) {
		...
		height = Math.max(hHint, Math.max(textSize.y, arrowSize.y) + 2
				* borderWidth);
		width = Math.max(wHint, Math.max(textSize.x + 2 * borderWidth, 0));
	 ...
	 * index is not selected, it remains deselected. Indices that are out of
	...
	Label getAssociatedLabel() {
		...
		for (int i = 0; i < siblings.length; i++) {
			if (siblings[i] == CReflectedCombo.this) {
	 ...
	 *                (ERROR_WIDGET_DISPOSED) when the widget has been disposed
	 * @exception SWTError
	 *                (ERROR_ERROR_CANNOT_GET_ITEM_HEIGHT) when the operation
	 *                fails
	 */
	public int getTextHeight() {
		checkWidget();
		return text.computeSize(SWT.DEFAULT, SWT.DEFAULT).y;// getLineHeight();
	}
	 ...
	 * an item is found that is equal to the search itenm. If no item is found,
	 * -1 is returned. Indexing is zero based.
	 * 
	 * @param string
	 *            the search item
	 * @param start
	...
	void initAccessible() {
		...
		getAccessible().addAccessibleTextListener(new AccessibleTextAdapter() {
			...
			public void getCaretOffset(AccessibleTextEvent e) {
				e.offset = 0;// text.getCaretPosition();
			}
		});
	...
	void internalLayout() {
		...
		arrow.setBounds(0, 0, 0, 0);// arrowSize.x, arrowSize.y);
	...
	void popupEvent(Event event) {
		...
		case SWT.Paint:
			...
			Rectangle listRect = list.getBounds();
			Color black = ResourceManager.getColor(SWT.COLOR_BLACK);
	 ...
	 * @exception SWTError
	 *                (ERROR_ITEM_NOT_MODIFIED) when the operation fails
	 */
